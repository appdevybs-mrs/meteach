...
Future<void> _showEnrollDialog(BuildContext context, {required BuildContext sheetCtx}) async {
// ✅ New controllers
...
// ✅ Build payment options depending on what exists for this course
final List<String> paymentOptions = [];
if ((course.pricePerMonth ?? 0) > 0) paymentOptions.add('Per month');
if ((course.pricePerLevel ?? 0) > 0) paymentOptions.add('Per level');
if (paymentOptions.isEmpty) paymentOptions.add('Not specified');

// ✅ Default payment selection
String paymentSelected = paymentOptions.first;

// ✅ Delivery options: prefer array, fallback to string
...
if (raw.isNotEmpty) {
// if it's "Online, In-person" split it
deliveryOptions = raw.split(',').map((e) => e.trim()).where((e) => e.isNotEmpty).toList();
}
}
if (deliveryOptions.isEmpty) deliveryOptions = ['Not specified'];

// ✅ Default delivery selection
String deliverySelected = deliveryOptions.first;

Future<bool> save() async {
final fullName = fullNameC.text.trim();
final phone = phoneC.text.trim();
final extra = extraInfoC.text.trim();

if (fullName.isEmpty || phone.isEmpty) {
messengerKey.currentState?.showSnackBar(
const SnackBar(content: Text('Please fill Full name and Phone.')),
);
return false;
}
...
if (dialogCtx.mounted) Navigator.of(dialogCtx).pop(); // close dialog
...
onPressed: () => _showEnrollDialog(context, sheetCtx: context),
