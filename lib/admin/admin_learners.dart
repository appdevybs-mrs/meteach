...
class _LearnerExpandedTabsState extends State<_LearnerExpandedTabs>
            ...
            builder: (context, snap) {
              ...
              return ListView.builder(
                ...
                itemBuilder: (context, i) {
                  ...
                  final hasDiagram = diagramUrl.isNotEmpty;
  ...
  void _openDiagramViewer(
    ...
    final startIdx = diagramItems.indexWhere((d) {
      final idx = allItems.indexWhere((item) {
        ...
        final diagramUrl = (item['diagramUrl'] ?? '').toString().trim();
        return rid == currentReportId && diagramUrl == d['url'];
      });
      return idx >= 0;
    });
    final initialIndex = startIdx >= 0 ? startIdx : 0;

    showDialog(
      context: context,
      barrierColor: Colors.black.withValues(alpha: 0.94),
      barrierDismissible: false,
      builder: (_) => _DiagramViewer(
  ...
  static Widget _metricChip(IconData icon, String label, int value, int? max) {
    final display = max != null ? '$value/$max' : '$value';
    return Container(
      ...
      child: Row(
        mainAxisSize: MainAxisSize.min,
        children: [
          Icon(icon, size: 14, color: AdminLearnersScreen.primaryBlue),
          const SizedBox(width: 4),
          Text(
            '$label $display',
            style: const TextStyle(
...
class _DiagramViewer extends StatefulWidget {
  final List<Map<String, String>> diagramItems;
  ...
  @override
  State<_DiagramViewer> createState() => _DiagramViewerState();
}

class _DiagramViewerState extends State<_DiagramViewer> {
      ...
      child: Stack(
        children: [
          PageView.builder(
            ...
            itemCount: widget.diagramItems.length,
            onPageChanged: (i) => setState(() => _currentPage = i),
            itemBuilder: (context, i) {
              final item = widget.diagramItems[i];
              final url = item['url'] ?? '';
            ...
            child: Container(
              ...
              child: Row(
                children: [
                  ...
                  Expanded(
                    child: Text(
                      widget.diagramItems.length > 1
                          ? '${_currentPage + 1} / ${widget.diagramItems.length}'
                          : '',
                      style: const TextStyle(
          ...
          Positioned(
            ...
            child: Container(
              ...
              child: Column(
                ...
                children: [
                  Text(
                    widget.diagramItems[_currentPage]['label'] ?? '',
                    maxLines: 1,
                    overflow: TextOverflow.ellipsis,
                    style: const TextStyle(
                      color: Colors.white,
                  ...
                  const SizedBox(height: 8),
                  if (widget.diagramItems.length > 1)
                    Row(
                      mainAxisAlignment: MainAxisAlignment.center,
                      children: List.generate(
                        widget.diagramItems.length,
                        (i) => Container(
                          width: 8,
                          height: 8,
