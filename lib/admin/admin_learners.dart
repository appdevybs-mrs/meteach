  ...
  class _LearnersListState extends State<_LearnersList> with AutomaticKeepAliveClientMixin {
    ...
    if (v is! Map) return builder(_PayFlag.ok);
          // We must check if ANY course is due.
          // But sessionsDone is based on class attendance -> we need to read classes/{classId}/attendance count.
          // We'll collect all classIds first, then listen to each (nested stream).
          final courseMaps = <Map<String, dynamic>>[];

          v.forEach((courseKey, courseVal) {
            if (courseVal is! Map) return;
            courseMaps.add(courseVal.map((k, vv) => MapEntry(k.toString(), vv)).cast<String, dynamic>());
          });

          // Helper to read classId from a course node
          String classIdOf(Map<String, dynamic> courseMap) {
            final cls = courseMap['class'];
            if (cls is! Map) return '';
            return (cls['class_id'] ?? '').toString().trim();
          }

          // Build list of classIds (unique)
          final classIds = <String>{};
          for (final cm in courseMaps) {
            final cid = classIdOf(cm);
            if (cid.isNotEmpty) classIds.add(cid);
          }

    if (classIds.isEmpty) return builder(_PayFlag.ok);
          // Listen to ALL those class attendance nodes and decide if due.
          return StreamBuilder<List<int>>(
            stream: StreamZip(
              classIds.map((cid) {
                final ref = _db.ref('classes/$cid/attendance');
                return ref.onValue.map((e) => (e.snapshot.value is Map) ? (e.snapshot.value as Map).length : 0);
              }).toList(),
            ),
            builder: (context, classCountsSnap) {
              final counts = classCountsSnap.data;

              if (counts == null || counts.length != classIds.length) {
                // if not ready -> don't mark due (safe)
                return builder(_PayFlag.ok);
              }

// For each course, compute the payment FLAG using taught sessions count of its class
    _PayFlag best = _PayFlag.ok;

// Priority: black > red > yellow > ok
    int rank(_PayFlag f) {
    switch (f) {
    case _PayFlag.black: return 3;
    case _PayFlag.red: return 2;
    case _PayFlag.yellow: return 1;
    case _PayFlag.ok:
    default: return 0;
    }
    }
    ...
    int index = 0;
    final classIdList = classIds.toList(); // stable order only inside this build
    final Map<String, int> taughtCountByClass = {};
    for (final cid in classIdList) {
    taughtCountByClass[cid] = counts[index++];
    }
    ...
    final sessionsPaidTotal = _LearnerExpandedTabsState._asInt(sumMap['sessionsPaidTotal']);
    final remindBeforeSession = _LearnerExpandedTabsState._asInt(sumMap['remindBeforeSession']);

    final classId = classIdOf(courseMap);
    if (classId.isEmpty) continue;

    final sessionsDone = taughtCountByClass[classId] ?? 0;
    ...
    // if black, it's the strongest signal, stop early
  ...
  class _LearnerExpandedTabsState extends State<_LearnerExpandedTabs> with SingleTickerProviderStateMixin {
    ...
    // ✅ Due helper (used in Payment tab)
    bool _isDue({
      required int sessionsPaidTotal,
      required int sessionsDone,
      required int remindBeforeSession,
    ...
    Widget _paymentPanel(BuildContext context) {
      ...
      // ✅ sessionsDone = number of UNIQUE attended sessions (by date) saved under this course
                        ...
                        builder: (context, snap) {
                          ...
                          return ListView.builder(
                            ...
                            itemBuilder: (context, i) {
                              ...
                              final left = (sessionsPaidTotal - sessionsDone) < 0 ? 0 : (sessionsPaidTotal - sessionsDone);
    ...
    Widget _attendanceTab(BuildContext context, List<String> keys) {
      ...
      // ✅ get classId from learner course node (teacher stamps this)
      final classId = (courseNode['class'] is Map)
          ? ((courseNode['class'] as Map)['class_id'] ?? '').toString().trim()
          : '';
      ...
      // learner-side attendance (may be partial, used only to show status per session)
      ...
      return FutureBuilder<DataSnapshot>(
        ...
        builder: (context, cs) {
          ...
          // ✅ canonical sessions taught = classes/{classId}/attendance
          return FutureBuilder<DataSnapshot>(
                ...
                children: [
                  ...
                  if (classSessions.isEmpty)
                    ...
                    ...classSessions.asMap().entries.map((entry) {
                      ...
                      // learner status if exists, else not registered (your rule: absent may not be written)
    ...
    static List<Map<String, dynamic>> _uniqueAttendanceByDate(dynamic attendance) {
      ...
      // date -> best session (latest updatedAt/createdAt)
